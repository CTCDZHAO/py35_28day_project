...
class TestWithDraw(unittest.TestCase,BaseTest):
    excel=HandlExcel(os.path.join(DATA_DIR,'apicases.xlsx'),'withdraw')
    ...
    def test_withdraw(self,item):
        ...
        # 执行sql查询余额
        stat_amount = self.db.find_one(sql)[0]
        ...
        try:
            ...
            my_log.error(e)
            if item['check_sql']:
                self.assertEqual(params['amount'],float(stat_amount-end_amount))
            # else:
